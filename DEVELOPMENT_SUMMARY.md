...
- 日程卡片可见性滑轨修复：
  - 右上角三枚状态图标改为独立分段滑轨，分别对应「显示给他人 / 不显示详细事项 / 隐藏」。
  - 点击滑轨只更新日程 `visibility`，不会触发卡片编辑；点击卡片其它区域才进入编辑。
- 日程纵向密度优化：
  - 默认小时高度从 92px 压缩到 66px，默认视口从约 6 小时调整到约 7 小时。
  - 降低卡片完整展示阈值并压缩标题、时间、地点字号/间距，让 100 分钟课程卡片更扁。
  - 同步下调缩放上下限和占用块阈值，保留双指缩放时的可读性降级。
- 日程手机拖动与纵轴再压缩：
  - 默认小时高度继续压缩到 58px，默认视口约 8 小时。
  - 日程画布单指拖动时阻止浏览器默认手势，提升手机端左右/上下拖动画布的稳定性。
  - 日程画布增加 grab/grabbing 交互反馈，并继续保留双指缩放。
- 社团板块追加精修：
...
- AI 模块移动端填满视口修复：
  - 将 `AppShell` 的 `<main>` 改为 `flex min-h-dvh flex-col`，使 `<section>` 上的 `flex-1` 能够生效并撑满视口高度。
  - `dvh`（dynamic viewport height）在移动端优于 `vh`，会自动适配浏览器地址栏的显示/隐藏。
  - 修复后聊天区域填满屏幕，输入框固定在底部，不再悬浮在页面中间。
  - 移除 `ChatPage.tsx` 中未使用的 `Menu` 导入（汉堡菜单已整合到 `ChatHeader` 内部）。
