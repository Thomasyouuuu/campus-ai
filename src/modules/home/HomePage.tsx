...
export function HomePage() {
  ...
  return (
    <AppShell maxWidth="max-w-[1500px]" contentClassName="gap-5 pt-4 sm:pt-5">
        <header className="liquid-glass overflow-hidden rounded-[28px] p-4 sm:rounded-[34px] sm:p-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-3 max-w-4xl text-[2rem] font-semibold leading-[1.02] tracking-normal sm:text-6xl">
                先把校园生活整理清楚，再遇见同频的人
              </h1>
            </div>
        ...
        <section className="paper-board rounded-[28px] p-2.5 sm:rounded-[34px] sm:p-4">
          ...
          <div className="relative -mt-px overflow-hidden rounded-b-[26px] rounded-tr-[26px] border border-white/70 bg-[#fff8dd]/72 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)] sm:rounded-b-[30px] sm:rounded-tr-[30px] sm:p-5">
            {board === "tools" ? (
              <ToolsBoard />
            ) : (
              <TagWorldBoard />
...
function BoardTab({
  ...
  return (
    <button
      className={`relative min-h-14 rounded-t-[24px] px-4 py-3 text-left transition sm:min-h-20 sm:rounded-t-[30px] sm:px-6 sm:py-5 ${
        active
      ...
      type="button"
    >
      <span className="block text-xl font-semibold tracking-normal sm:text-5xl">
        {label}
      </span>
    </button>
  );
...
function ToolsBoard() {
  return (
    <div className="grid gap-5">
      ...
      <section className="min-h-[520px] sm:min-h-[720px]">
        <HomeDashboard />
      </section>
    </div>
  );
    ...
    <section className="grid gap-3 lg:grid-cols-2">
      {quickCards.map((card) => (
        ...
        >
          ...
          <h2 className="mt-2 text-2xl font-semibold leading-none sm:text-3xl">
                {card.title}
              </h2>
      ...
      <div className="liquid-glass rounded-[28px] p-4 sm:rounded-[32px] sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold leading-none sm:text-4xl">
              今天先看这些
            </h2>
          </div>
...
function MiniSchedulePreview() {
  ...
  return (
    <section className="paper-board mt-5 overflow-hidden rounded-[26px] p-2.5 sm:mt-6 sm:rounded-[30px] sm:p-3">
      <div className="flex items-center justify-between px-2 pb-3">
        <div>
          ...
          <h3 className="mt-1 text-xl font-semibold sm:text-2xl">三日缩略视图</h3>
        </div>
      ...
      <div className="grid h-[320px] grid-cols-[54px_1fr] overflow-hidden rounded-[22px] border border-white/64 bg-white/18 sm:h-[360px] sm:grid-cols-[64px_1fr] sm:rounded-[24px]">
        <div className="relative border-r border-white/58 bg-[#fff8dd]/42">
          {hours.map((hour, index) => (
            <span
              className="absolute left-2 text-[11px] font-semibold text-amber-950/38 sm:left-3 sm:text-xs"
              key={hour}
        ...
        <div className="relative grid grid-cols-3">
          {days.map((day, dayIndex) => (
            <div
              ...
              key={day.label}
            >
              <div className="sticky top-0 z-10 border-b border-white/52 bg-[#fff8dd]/44 px-2 py-2.5 backdrop-blur-md sm:px-3 sm:py-3">
  ...
  return (
    <section className="liquid-glass grid min-h-[620px] place-items-center rounded-[32px] p-8 text-center">
      <div className="max-w-lg">
        ...
        <h2 className="mt-5 text-3xl font-semibold sm:text-4xl">Tag World</h2>
        <p className="mt-3 text-sm leading-6 text-slate-600">
